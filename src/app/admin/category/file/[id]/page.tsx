...
export default function Page() {
    ...
    const {data: data, isLoading: isLoading} = useQuery({
        queryKey: [`files`],
        queryFn: () => getFiles({model_id: Number(id), model_type: "category"}),
        staleTime: 5000,
    });

    async function submit(e: FormData) {
        let response = await upload({model_id: Number(id), file: e.get("file") as File, model_type: "category"})
        if (response?.success) {
    ...
    return (<>
        <Breadcrump breadcrumb={[
            ...
            {
                title: "ویرایش فایل دسته بندی",
                href: "category/file/" + id
            }
        ]}/>
        <Panel>
            <ProductTab id={id + ""}/>
                            ...
                            <div className="flex flex-col justify-center items-center gap-y-4 ">
                                <div
                                    ...
                                    <NcImage
                                        ...
                                        src={`${process.env.NEXT_PUBLIC_IMAGE_BASE_URL}/category/file/${item.path}`}
                                    />
                                </div>
                                <span>
                                {item.path}
